...
class Defuddle {
    ...
    parse() {
        ...
        if (schemaText && this.countHtmlWords(schemaText) > result.wordCount * 1.5) {
            ...
            const safeDoc = liveDoc.cloneNode(true);
    ...
    _serializeFallbackBody() {
        if (!this.doc.body)
            return '';
        const safeBody = this.doc.body.cloneNode(true);
        this._stripUnsafeElements(safeBody);
        return this.resolveContentUrls((0, dom_1.serializeHTML)(safeBody));
    }
    /**
     * Remove dangerous elements and attributes from the given body element.
     */
    _stripUnsafeElements(body) {
        if (!body)
            return;
        ...
        for (const el of allElements) {
            for (const attr of Array.from(el.attributes)) {
                ...
                if (name.startsWith('on')) {
                    ...
                    if ((0, dom_1.isDangerousUrl)(attr.value, allowInlineImage)) {
                        el.removeAttribute(attr.name);
                    }
                }
            }
    ...
    _deduplicateImages(body) {
        for (const figure of body.querySelectorAll('figure')) {
            ...
            for (const [key, group] of groups) {
                ...
                if (key !== null && group.every(img => img.getAttribute('src') === group[0].getAttribute('src')))
                    continue;
        ...
        for (let i = 0; i < imgs.length - 1; i++) {
            ...
            if (!img.parentElement)
                continue; // already removed as a loser
            if (img.closest('noscript') || img.closest('figure'))
                continue;
            ...
            if (!other.parentElement)
                continue;
            if (other.closest('noscript') || other.closest('figure'))
                continue;
            if ((other.getAttribute('alt') || '').trim() !== alt)
                continue;
            const otherSrc = other.getAttribute('src') || '';
            if (!otherSrc || otherSrc.startsWith('data:'))
                continue;
            if (otherSrc === src)
                continue; // legitimate repeat (same image twice)
            // Require true adjacency: only whitespace/wrapper markup between them.
            if (!this._noVisibleContentBetween(img, other))
                continue;
        ...
        for (const img of Array.from(body.querySelectorAll('img'))) {
            if (!img.parentElement)
                continue;
            if (img.closest('a, figure, noscript'))
                continue;
            const src = img.getAttribute('src') || '';
            if (!src || src.startsWith('data:'))
                continue;
     ...
     */
    _removeCoverImage(body, metadataImage) {
        if (!metadataImage)
            return;
        ...
        for (const img of body.querySelectorAll('img')) {
            ...
            if (!src || src.startsWith('data:'))
                continue;
            if (this._normalizeSrc(src) !== metaNorm)
                continue;
     ...
     */
    _normalizeAttributes(body) {
        const renames = [['srcSet', 'srcset']];
    ...
    _resolveNoscriptImages(body) {
        ...
        for (const noscript of noscripts) {
            ...
            if (!noscriptImg) {
                ...
                if (!html.includes('<img'))
                    continue;
                const fragment = (0, dom_1.parseHTML)(this.doc, html);
            ...
            if (!matched && this._isLazyImageContext(noscript)) {
                ...
                if (!hasRealImage) {
                    const promotedImg = noscriptImg.cloneNode(true);
     ...
     */
    _isLazyImageContext(noscript) {
        if (noscript.closest('figure'))
            return true;
        const parent = noscript.parentElement;
        if (parent) {
            for (const sibling of parent.children) {
                if (sibling === noscript)
                    continue;
                if ((0, dom_1.getClassName)(sibling).toLowerCase().includes('lazy'))
                    return true;
            }
            const parentCls = (0, dom_1.getClassName)(parent).toLowerCase();
    ...
    async fetchAsyncVariables() {
        ...
        try {
            ...
            const extractorOpts = { includeReplies: this.options.includeReplies ?? 'extractors', language: this.options.language, fetch: this.options.fetch };
        ...
        try {
            ...
            profileStep('removeEyebrowLabel', () => {
                if (options.removeContentPatterns && mainContent) {
                    (0, content_patterns_1.removeEyebrowLabel)(mainContent, this.debug, debugRemovals);
                }
            });
            ...
            profileStep('removeByContentPattern', () => {
                if (options.removeContentPatterns && mainContent) {
                    const url = this.options.url || this.doc.URL || '';
                    (0, content_patterns_1.removeByContentPattern)(mainContent, this.debug, url, metadata.title || '', metadata.description || '', debugRemovals);
                }
            });
            ...
            this._stripUnsafeElements(mainContent);
    ...
    findTableBasedContent(doc) {
        ...
        const hasTableLayout = tables.some(table => {
            ...
            const tableClass = (0, dom_1.getClassName)(table).toLowerCase();
            return width > 400 ||
        ...
        const bestCell = scoring_1.ContentScorer.findBestElement(cells);
        if (!bestCell)
            return null;
        ...
        const bestCellWords = (0, utils_1.countWords)(bestCell.textContent || '');
        const bodyWords = (0, utils_1.countWords)((doc.body || doc.documentElement).textContent || '');
    ...
    getElementSelector(element) {
        ...
        while (current && current !== this.doc.documentElement) {
            ...
            if (current.id) {
                ...
                const safe = (0, dom_1.getClassName)(current).trim().split(/\s+/)
    ...
    adoptExternalFootnotes(mainContent, root) {
        const body = root.body || root;
        if (!body || mainContent === body)
            return;
        body.querySelectorAll('div, section, aside').forEach((el) => {
            ...
            if (!heading || !footnotes_1.FOOTNOTE_SECTION_RE.test(heading.textContent?.trim() || ''))
                return;
    ...
    _sanitizeExtractorHtml(html) {
        ...
        this.resolveRelativeUrls(container);
        (0, standardize_1.standardizeExtractorOutput)(container, this.debug);
        return (0, dom_1.serializeHTML)(container);
    }
    /**
...
//   Next.js:     ?w=1300
Defuddle._urlWidthPattern = /(?:width[=:/]|[/,?&]w[_:=])(\d+)/;
