...
exports.standardizeExtractorOutput = standardizeExtractorOutput;
exports.standardizeContent = standardizeContent;
exports.removeOrphanedDividers = removeOrphanedDividers;
const constants_1 = require("./constants");
const math_1 = require("./elements/math");
const math_base_1 = require("./elements/math.base");
 ...
 */
function standardizeExtractorOutput(element, debug = false) {
    _debug = debug;
...
function standardizeContent(element, metadata, doc, debug = false, subProfile) {
    _debug = debug;
    ...
    if (!debug) {
        ...
        step('removePermalinkAnchors', () => (0, headings_1.removePermalinkAnchors)(element));
        ...
        step('unwrapSpecialLinks', () => {
            ...
            Array.from(element.querySelectorAll('a')).forEach(link => {
                ...
                while (heading.firstChild)
                    innerLink.appendChild(heading.firstChild);
...
function standardizeSpaces(element) {
    const processNode = (node) => {
        ...
        if ((0, utils_1.isElement)(node)) {
            ...
            if (tag === 'pre' || tag === 'code' || (0, utils_1.isSVGElement)(node)) {
                return;
            }
        }
        // Process text nodes
...
function removeOrphanedDividers(element) {
    ...
    for (const hr of element.querySelectorAll('hr')) {
        ...
        while (node) {
            ...
            if ((0, utils_1.isElement)(node) && node.tagName === 'HR') {
...
function standardizeHeadings(element, title, doc) {
    ...
    if (h2s.length > 0) {
        ...
        for (const a of firstH2.querySelectorAll('a')) {
            if ((0, headings_1.isPermalinkAnchor)(a))
                permalinkText += a.textContent || '';
        }
        const firstH2Text = (0, utils_1.normalizeText)((firstH2.textContent || '').replace(permalinkText, ''));
...
function filterExtractorClasses(value) {
    return value.split(/\s+/).filter(comments_1.isExtractorClass).join(' ');
}
/**
 ...
 */
function stripUnwantedAttributes(element, debug, extractorOutput = false) {
 ...
 */
function unwrapLayoutTables(element) {
    ...
    for (const table of tables) {
        ...
        if (table.querySelector('thead, tfoot, th, caption'))
            continue;
        ...
        const children = Array.from(cell.children).filter(c => c.textContent?.trim());
        if (children.length === 1 && constants_1.BLOCK_LEVEL_ELEMENTS.has(children[0].tagName.toLowerCase())) {
            table.replaceWith(children[0]);
            count++;
        }
    }
    (0, utils_1.logDebug)(_debug, 'Unwrapped layout tables:', count);
}
 ...
 */
function replaceCustomElements(element, doc) {
    const customElements = Array.from(element.querySelectorAll('*')).filter(el => el.tagName.includes('-')
        && !constants_1.INLINE_ELEMENTS.has(el.tagName.toLowerCase())
        && !(0, utils_1.isSVGElement)(el)).reverse();
    let replacedCount = 0;
    ...
    (0, utils_1.logDebug)(_debug, 'Replaced custom elements with divs:', replacedCount);
}
 ...
 */
function standardizeDropCaps(element) {
    ...
    if (count > 0)
        element.normalize();
    (0, utils_1.logDebug)(_debug, 'Standardized drop caps:', count);
}
/**
 ...
 */
function convertDataAsSpans(element, doc) {
    ...
    for (const span of spans) {
        ...
        if (!DATA_AS_ALLOWED.has(target))
            continue;
        const replacement = doc.createElement(target);
        (0, dom_1.transferContent)(span, replacement);
        span.replaceWith(replacement);
        convertedCount++;
    }
    (0, utils_1.logDebug)(_debug, 'Converted data-as spans:', convertedCount);
}
/**
 ...
 */
function convertBlockSpans(element, doc) {
    let convertedCount = 0;
    const spans = Array.from(element.querySelectorAll('span[class*="block"], span[style*="block"]'));
    for (const span of spans) {
        if (!span.parentNode)
            continue;
        const isBlock = TW_BLOCK_RE.test((0, dom_1.getClassName)(span)) ||
        ...
        if (!span.textContent?.trim())
            continue;
        const p = doc.createElement('p');
        (0, dom_1.transferContent)(span, p);
        span.replaceWith(p);
        convertedCount++;
    }
    (0, utils_1.logDebug)(_debug, 'Converted block spans to paragraphs:', convertedCount);
}
 ...
 */
function resolveSvgColors(element, doc) {
    ...
    const resolveVar = (value, svgParent) => {
        ...
        if (varMatch) {
            ...
            if (twMatch) {
                const hex = constants_1.TAILWIND_COLORS[twMatch[1]]?.[twMatch[2]];
                if (hex)
                    return hex;
            }
            ...
            if (name.endsWith('-white'))
                return '#fff';
            // Semantic fallbacks
            if (name.includes('background') || name.includes('card') || name.includes('surface') || name.includes('bg'))
                return 'Canvas';
            if (name.includes('border') || name.includes('divider') || name.includes('separator'))
                return '#ccc';
            if (name.includes('muted') || name.includes('subtle') || name.includes('secondary') || name.includes('placeholder'))
                return '#888';
        }
        return 'currentColor';
    };
    for (const svg of Array.from(svgs)) {
        ...
        for (const el of allEls) {
            for (const attrName of SVG_COLOR_ATTRS) {
                ...
                if (!val || (!val.includes('var(') && !val.includes('light-dark(')))
                    continue;
 ...
 */
function applySvgFallbackStyles(svg) {
    ...
    for (const el of allEls) {
        const tag = el.tagName.toLowerCase();
        if (!SVG_FILLED_TAGS.has(tag))
            continue;
        if (!el.getAttribute('class'))
            continue;
        if (el.closest(SVG_NON_RENDERED_ANCESTOR))
            continue;
...
function resolveTailwindClasses(el) {
    ...
    for (const token of tokens) {
        let match = token.match(constants_1.TW_COLOR_CLASS_RE);
        if (match) {
            const [, prop, color, shade, opacity] = match;
            const hex = constants_1.TAILWIND_COLORS[color]?.[shade];
            if (hex) {
                if (opacity) {
        ...
        match = token.match(constants_1.TW_SPECIAL_CLASS_RE);
        if (match) {
            el.setAttribute(match[1], constants_1.TAILWIND_SPECIAL[match[2]]);
            continue;
        }
        match = token.match(constants_1.TW_ARBITRARY_RE);
...
function stripExtraBrElements(element) {
    ...
    for (const br of remainingBrs) {
        ...
        if (!parent)
            continue;
        if (br.closest('pre, code'))
            continue;
        const parentTag = parent.tagName.toLowerCase();
        // Case 1: <br> (or consecutive group) between block-level siblings
        // e.g. </p><br><p>, </figure><br><br><p>, </h2><br><p>
        if (constants_1.BLOCK_LEVEL_ELEMENTS.has(parentTag) || parentTag === 'body') {
            const group = [br];
            let scan = skipWhitespace(br, 'next');
            while (scan && (0, utils_1.isElement)(scan) && scan.tagName.toLowerCase() === 'br') {
                group.push(scan);
            ...
            const prevIsBlock = prev && (0, utils_1.isElement)(prev) && constants_1.BLOCK_LEVEL_ELEMENTS.has(prev.tagName.toLowerCase());
            const nextIsBlock = next && (0, utils_1.isElement)(next) && constants_1.BLOCK_LEVEL_ELEMENTS.has(next.tagName.toLowerCase());
        ...
        if (constants_1.BLOCK_LEVEL_ELEMENTS.has(parentTag)) {
...
function skipWhitespace(node, direction) {
    ...
    while (sibling && (0, utils_1.isTextNode)(sibling) && !sibling.textContent?.trim()) {
...
function isFootnoteRef(node) {
    return (0, utils_1.isElement)(node) &&
        node.tagName.toLowerCase() === 'sup' &&
        (node.getAttribute('id') || '').startsWith('fnref:');
}
...
function standardizeElements(element, doc, subProfile) {
    ...
    stepSE('wrapRawLatexDelimiters', () => (0, math_base_1.wrapRawLatexDelimiters)(element, doc));
    ...
    stepSE('convertLatexImages', () => {
        for (const img of Array.from(element.querySelectorAll('img[src]'))) {
            ...
            if (!src)
                continue;
            let latex = (0, math_base_1.extractLatexFromImageSrc)(src);
            // Fall back to alt text if it contains LaTeX commands
            if (!latex) {
                const alt = img.getAttribute('alt') || '';
                if (math_base_1.LOOKS_LIKE_LATEX_RE.test(alt)) {
                    latex = alt;
                }
            }
            if (!latex)
            ...
            const mathEl = (0, math_1.createCleanMathEl)(null, latex, isBlock, doc);
    ...
    ELEMENT_STANDARDIZATION_RULES.forEach(rule => {
        ...
        stepSE(selectorKey, () => {
            if (rule.fastCheck && !element.querySelector(rule.fastCheck))
                return;
            let elements;
            try {
    ...
    Array.from(element.querySelectorAll('code > pre')).forEach(pre => {
        ...
        if (!outerCode || outerCode.tagName !== 'CODE')
            return;
