...
exports.FOOTNOTE_SECTION_RE = void 0;
...
exports.FOOTNOTE_SECTION_RE = /^(foot\s*notes?|end\s*notes?|notes?|references?)$/i;
...
class FootnoteHandler {
    ...
    addFootnote(state, id, content, explicitNum) {
        if (!id || state.processedIds.has(id))
            return false;
    ...
    collectFootnotes(element) {
        const state = { footnotes: {}, processedIds: new Set(), count: 1 };
        ...
        footnoteLists.forEach((list) => {
            ...
            if (list.matches('div.footnotes-footer')) {
                ...
                footnoteDivs.forEach((div) => {
                    ...
                    if (backLink)
                        backLink.remove();
                    const text = (0, dom_1.serializeHTML)(clone).replace(/^\s*\.\s*/, '');
    ...
    tryDataTypeFootnotes(element, state) {
    ...
    tryGenericIdDetection(element, state) {
        ...
        element.querySelectorAll('a[href*="#"]').forEach((a) => {
            ...
            if (!FOOTNOTE_MARKER_RE.test(text))
                return;
    ...
    tryWordExport(element, state) {
        ...
        pairs.forEach(({ num, anchor }) => {
            ...
            if (state.processedIds.has(originalId))
                return;
    ...
    tryGoogleDocs(element, state) {
        const gdocPairs = [];
        element.querySelectorAll('p[id^="ftnt"]').forEach((p) => {
        ...
        if (prev && /^h[1-6]$/.test(prev.tagName.toLowerCase()) && exports.FOOTNOTE_SECTION_RE.test(prev.textContent?.trim() || '')) {
    ...
    tryLooseFootnotes(element, state) {
    ...
    tryClassFootnote(element, state) {
    ...
    tryLabeledSection(element, state) {
        const containers = element.querySelectorAll('div, section, aside');
        for (const container of Array.from(containers)) {
            const className = (0, dom_1.getClassName)(container);
            const id = container.id || '';
            if (!/footnote/i.test(className) && !/footnote/i.test(id))
                continue;
            const heading = container.querySelector('h1, h2, h3, h4, h5, h6');
            if (!heading || !exports.FOOTNOTE_SECTION_RE.test(heading.textContent?.trim() || ''))
                continue;
            // Definitions are either numbered paragraphs or an ordered list.
            if (!this.collectSectionParagraphs(container, state)
                && !this.collectSectionList(container, state))
                continue;
            this.pendingRemovals.push(container);
            break;
        }
    ...
    tryBrSeparatedFootnotes(element, state) {
        const linkedFragments = new Set();
        element.querySelectorAll('a[href^="#"]').forEach((a) => {
            if (FOOTNOTE_MARKER_RE.test(a.textContent?.trim() || '')) {
                const fragment = getHrefFragment(a);
                if (fragment)
                    linkedFragments.add(fragment);
            }
        });
        if (linkedFragments.size < 2)
            return;
        for (const block of Array.from(element.querySelectorAll('p, div'))) {
            ...
            for (const nodes of this.splitOnBreaks(block)) {
                ...
                if (!id || !linkedFragments.has(id))
                    continue;
                defs.push({ id, nodes: nodes.filter((n) => n !== anchor) });
            }
            if (defs.length < 2)
                continue;
            defs.forEach(({ id, nodes }) => {
                const contentDiv = element.ownerDocument.createElement('div');
                nodes.forEach((node) => contentDiv.appendChild(node.cloneNode(true)));
    ...
    splitOnBreaks(block) {
        ...
        for (const node of Array.from(block.childNodes)) {
            if ((0, utils_1.isElement)(node) && node.tagName.toLowerCase() === 'br') {
                if (current.length)
    ...
    collectSectionParagraphs(container, state) {
        const paragraphs = [];
        container.querySelectorAll('p').forEach((p) => {
            const num = this.parseFootnoteNum(p);
            if (num !== null)
                paragraphs.push({ num, el: p });
        });
    ...
    collectSectionList(container, state) {
    ...
    findLooseFootnoteParagraphs(element) {
        ...
        if (trailingNumbered.length >= 2 && this.crossValidate(element, trailingNumbered)) {
            ...
            if (prev) {
                ...
                if (/^h[1-6]$/.test(prevTag) && exports.FOOTNOTE_SECTION_RE.test(prev.textContent?.trim() || '')) {
    ...
    isBackrefLink(el) {
        ...
        if (BACKREF_SYMBOLS_RE.test(text) || !!el.classList?.contains('footnote-backref'))
            return true;
    ...
    extractListItemIdAndContent(li) {
        ...
        for (const prefix of ['bib.bib', 'fn:', 'fn']) {
            if (rawId.startsWith(prefix))
                return { id: rawId.slice(prefix.length), content: li };
        }
        ...
        if (containers.length === 0) {
            ...
            if (footrefs.length > 0) {
                ...
                footrefs.forEach((label) => {
                    ...
                    if (!sibling || sibling.tagName !== 'SPAN' || !sibling.classList?.contains('sidenote'))
                        return;
    ...
    standardizeFootnotes(element) {
        ...
        footnoteInlineReferences.forEach((el) => {
            ...
            if (!footnoteId) {
                ...
                const href = el.getAttribute('href');
                if (href)
                    footnoteId = href.replace(/^[#]/, '').toLowerCase();
            }
            if (footnoteId) {
                ...
                if (footnoteEntry) {
                    ...
                    if (isSup && supGroups.get(container)?.some((r) => r.footnoteNumber === footnoteNumber)) {
                        return;
                    }
        ...
        if (unmatchedFootnotes.length > 0) {
            ...
            const isInsideFootnotes = (el) => el.closest('[id^="fnref:"]') || el.closest('#footnotes') ||
                this.pendingRemovals.some((g) => g.contains(el));
        ...
        this.pendingRemovals.forEach((el) => { if (el.parentNode)
            el.remove(); });
