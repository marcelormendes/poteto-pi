...
exports.removeEyebrowLabel = removeEyebrowLabel;
exports.removeByContentPattern = removeByContentPattern;
const constants_1 = require("../constants");
const utils_1 = require("../utils");
const content_boundary_1 = require("../content-boundary");
...
const CONTENT_ELEMENT_NO_IMG_SELECTOR = constants_1.CONTENT_ELEMENT_SELECTOR.replace(/img, picture, /, '');
const EMAIL_PATTERN = /[\w.-]+@[\w.-]+\.\w+/;
const PHONE_PATTERN = /\(?\d{3}\)?[\s.‑–-]?\d{3}[\s.‑–-]?\d{4}/;
const HEADING_TAG_PATTERN = /^H[1-6]$/;
const HEADING_SELECTOR = 'h1, h2, h3, h4, h5, h6';
function isOrContainsHeading(el) {
...
function isNewsletterElement(el, maxWords) {
    ...
    if (words < 2 || words > maxWords)
        return false;
    if (el.querySelector(constants_1.CONTENT_ELEMENT_SELECTOR))
        return false;
...
function hasFollowingProse(el, minWords = 25) {
    let sibling = el.nextElementSibling;
    while (sibling) {
        if (sibling.tagName === 'P' && (0, utils_1.countWords)(sibling.textContent || '') >= minWords)
            return true;
        for (const p of sibling.querySelectorAll('p')) {
            if ((0, utils_1.countWords)(p.textContent || '') >= minWords)
                return true;
        }
...
function precedingProseWords(el, mainContent) {
    let words = 0;
    let node = el;
    while (node && node !== mainContent) {
        ...
        while (sibling) {
            if (sibling.tagName === 'P') {
                words += (0, utils_1.countWords)(sibling.textContent || '');
            }
            else {
                for (const p of sibling.querySelectorAll('p'))
                    words += (0, utils_1.countWords)(p.textContent || '');
            }
...
function walkUpToWrapper(el, text, mainContent) {
    ...
    while (target.parentElement && target.parentElement !== mainContent) {
        if ((target.parentElement.textContent?.trim() || '') !== text)
            break;
...
function removeTrailingWithCascade(target, mainContent, debug, debugRemovals) {
    const ancestors = [];
...
function walkUpIsolated(el, mainContent) {
    ...
    while (target.parentElement && target.parentElement !== mainContent) {
        ...
        while (sib) {
            precedingWords += (0, utils_1.countWords)(sib.textContent || '');
...
function removeThinPrecedingSection(target, debug, debugRemovals) {
    ...
    if (!prevSib)
        return;
    if ((0, utils_1.countWords)(prevSib.textContent || '') >= 50)
        return;
    if (prevSib.querySelector(constants_1.CONTENT_ELEMENT_SELECTOR))
        return;
    ...
    if (debug && debugRemovals) {
        debugRemovals.push({ step: 'removeByContentPattern', reason: 'thin CTA section', text: (0, utils_1.textPreview)(prevSib) });
    }
    prevSib.remove();
}
/**
 ...
 */
function removeHeroHeader(mainContent, contentStart, debug, debugRemovals) {
    const timeElements = mainContent.querySelectorAll('time');
    if (timeElements.length === 0)
        return;
    for (const time of timeElements) {
        // Must sit above the content body — i.e. pre-content metadata.
        if (!(0, content_boundary_1.isAboveContentStart)(time, contentStart))
            continue;
        ...
        while (current && current !== mainContent) {
            ...
            if (hasHeadingAndTime) {
                ...
                for (const el of metadataEls) {
                    metadataWords += (0, utils_1.countWords)(el.textContent || '');
                }
        ...
        if (bestBlock) {
            if (debug && debugRemovals) {
                debugRemovals.push({
                    ...
                    text: (0, utils_1.textPreview)(bestBlock)
                });
            }
            bestBlock.remove();
            return;
...
function isBreadcrumbList(list) {
    ...
    if (listLinks.length < 1 || listLinks.length >= listItems.length)
        return false;
    if (list.querySelector('img, p, figure, blockquote'))
        return false;
    ...
    for (const item of listItems) {
        if ((0, utils_1.countWords)(item.textContent || '') > 8)
            return false;
    }
    ...
    for (const a of listLinks) {
        ...
        if (((a.textContent || '').trim().split(/\s+/).filter(Boolean).length) > 5)
...
function removeEyebrowLabel(mainContent, debug, debugRemovals) {
    ...
    if (/[.!?]/.test(text))
        return;
    if (CONTENT_DATE_PATTERN.test(text))
        return;
    if (prev.querySelector('img, picture, video, iframe, figure, table, pre, code, time, [datetime], ' +
        'h1, h2, h3, h4, h5, h6, ul, ol, blockquote'))
        return;
    if (debug && debugRemovals) {
        debugRemovals.push({ step: 'removeEyebrowLabel', reason: 'eyebrow label', text: (0, utils_1.textPreview)(prev) });
    }
    prev.remove();
}
function removeByContentPattern(mainContent, debug, url, title, description, debugRemovals) {
    ...
    const contentStart = (0, content_boundary_1.findContentStart)(mainContent, title);
    const isPreContent = (el) => (0, content_boundary_1.isAboveContentStart)(el, contentStart);
    const normalizedTitle = (0, utils_1.normalizeText)(title);
    const normalizedDesc = (0, utils_1.normalizeText)(description);
    ...
    if (firstList && isBreadcrumbList(firstList)) {
        ...
        if (debug && debugRemovals) {
            debugRemovals.push({ step: 'removeByContentPattern', reason: 'breadcrumb navigation list', text: (0, utils_1.textPreview)(target) });
        }
        target.remove();
    }
    ...
    if (firstH1) {
        for (const link of mainContent.querySelectorAll('a[href]')) {
            if (!link.parentNode)
                continue;
            if (!(link.compareDocumentPosition(firstH1) & 4))
                continue;
            ...
            if ((0, utils_1.countWords)(text) > 25)
                continue;
            if (/[.!?]\s/.test(text))
                continue;
            if (debug && debugRemovals) {
                debugRemovals.push({ step: 'removeByContentPattern', reason: 'promotional banner link', text: (0, utils_1.textPreview)(link) });
            }
            link.remove();
        }
    }
    ...
    for (const media of mainContent.querySelectorAll('audio, video')) {
        if (!media.parentNode)
            continue;
        if (!media.getAttribute('src') && !media.querySelector('source'))
            continue;
        let container = media;
        while (container.parentElement && container.parentElement !== mainContent) {
            if ((0, utils_1.countWords)(container.parentElement.textContent?.trim() || '') > 25)
                break;
        ...
        const isPreContentPlayer = !isListenWidget && isPreContent(container) &&
            (0, utils_1.countWords)(containerText) <= 25;
        if (isListenWidget || isPreContentPlayer) {
            if (debug && debugRemovals) {
                debugRemovals.push({ step: 'removeByContentPattern', reason: 'audio player widget', text: (0, utils_1.textPreview)(container) });
            }
            container.remove();
        }
    }
    ...
    for (const list of mainContent.querySelectorAll('ul, ol')) {
        ...
        if (listPos < 0 || listPos > contentText.length * 0.3)
            continue;
        const links = Array.from(list.querySelectorAll('a[href]'));
        if (links.length < 3)
            continue;
        if (list.querySelector(constants_1.CONTENT_ELEMENT_SELECTOR))
            continue;
        ...
        if (anchorCount < 3 || anchorCount / links.length < 0.8)
            continue;
        let target = list;
        while (target.parentElement && target.parentElement !== mainContent &&
            target.parentElement.children.length === 1) {
        ...
        if (prevEl && HEADING_TAG_PATTERN.test(prevEl.tagName)) {
            ...
            if (/^(?:table of )?contents$|^on this page$|^in this (?:article|guide|post)$/i.test(hText)) {
                if (debug && debugRemovals) {
                    debugRemovals.push({ step: 'removeByContentPattern', reason: 'table of contents heading', text: (0, utils_1.textPreview)(prevEl) });
                }
                prevEl.remove();
            }
        }
        ...
        if (debug && debugRemovals) {
            debugRemovals.push({ step: 'removeByContentPattern', reason: 'table of contents', text: (0, utils_1.textPreview)(target) });
        }
        target.remove();
        if (prevSib?.tagName === 'HR')
            prevSib.remove();
        if (nextSib?.tagName === 'HR')
            nextSib.remove();
        break;
    }
    ...
    for (const el of candidates) {
        ...
        const getPos = () => { if (pos === -2)
            pos = contentText.indexOf(text); return pos; };
        ...
        if (TIMEZONE_WIDGET_PATTERN.test(text) && getPos() <= 300) {
            ...
            if (debug && debugRemovals) {
                debugRemovals.push({ step: 'removeByContentPattern', reason: 'timezone widget', text: (0, utils_1.textPreview)(target) });
            }
            target.remove();
            continue;
        }
        ...
        if (words === 1 && PINNED_LABEL_PATTERN.test(text)) {
            if (debug && debugRemovals) {
                debugRemovals.push({ step: 'removeByContentPattern', reason: 'pinned label', text: (0, utils_1.textPreview)(el) });
            }
            el.remove();
            continue;
        }
        ...
        for (const [normalized, reason] of [
            ...
            if (normalized && words >= 3 && isPreContent(el) &&
                (0, utils_1.normalizeText)(text) === normalized) {
                if (debug && debugRemovals) {
                    debugRemovals.push({ step: 'removeByContentPattern', reason, text: (0, utils_1.textPreview)(el) });
                }
                el.remove();
                break;
            }
        ...
        if ((tag === 'DIV' || tag === 'P') && words >= 1 && words <= 10 && (hasDate || RELATIVE_TIME_PATTERN.test(text)) && !METADATA_LABEL_PATTERN.test(text) && !/[.!?]/.test(text) && isPreContent(el)) {
            if (!Array.from(el.querySelectorAll('p, h1, h2, h3, h4, h5, h6')).some(b => (0, utils_1.countWords)(b.textContent || '') > 8)) {
                if (debug && debugRemovals) {
                    debugRemovals.push({ step: 'removeByContentPattern', reason: 'article metadata header block', text: (0, utils_1.textPreview)(el) });
                }
                el.remove();
                continue;
            }
        ...
        if (tag === 'DIV' && words >= 1 && words <= 5 && !/[.!?]/.test(text) && isPreContent(el) && el.querySelector('img')) {
            ...
            if (links.length > 0) {
                ...
                if (linkTextLen / (text.length || 1) >= 0.8) {
                    if (debug && debugRemovals) {
                        debugRemovals.push({ step: 'removeByContentPattern', reason: 'category badge', text: (0, utils_1.textPreview)(el) });
                    }
                    el.remove();
                    continue;
                }
        ...
        if (!bylineFound && STARTS_WITH_BY_PATTERN.test(text) && words >= 2 && !/[.!?]$/.test(text) && isPreContent(el)) {
            ...
            if (debug && debugRemovals) {
                debugRemovals.push({ step: 'removeByContentPattern', reason: 'author byline', text: (0, utils_1.textPreview)(target) });
            }
            target.remove();
            bylineFound = true;
            continue;
        ...
        if (CONTENT_READ_TIME_PATTERN.test(text) &&
            ...
            if (cleaned.trim().length === 0) {
                ...
                if (debug && debugRemovals) {
                    debugRemovals.push({ step: 'removeByContentPattern', reason: 'read time metadata', text: (0, utils_1.textPreview)(target) });
                }
                target.remove();
                continue;
            }
        ...
        if (!authorDateFound && words >= 2 && words <= 10 && hasDate && !METADATA_LABEL_PATTERN.test(text) && isPreContent(el)) {
            ...
            if (residual) {
                ...
                if (nameWords.length >= 1 && nameWords.length <= 4 && nameWords.every(w => BYLINE_UPPERCASE_PATTERN.test(w))) {
                    const target = walkUpToWrapper(el, text, mainContent);
                    if (debug && debugRemovals) {
                        debugRemovals.push({ step: 'removeByContentPattern', reason: 'author date metadata', text: (0, utils_1.textPreview)(target) });
                    }
                    target.remove();
        ...
        if (hasDate && words <= 5 && isPreContent(el)) {
            ...
            if (residual.length === 0) {
                const target = walkUpToWrapper(el, text, mainContent);
                if (debug && debugRemovals) {
                    debugRemovals.push({ step: 'removeByContentPattern', reason: 'standalone date metadata', text: (0, utils_1.textPreview)(target) });
                }
                target.remove();
                continue;
            }
    ...
    for (const list of metadataLists) {
        ...
        if (items.length < minItems || items.length > 8)
            continue;
        ...
        if (prevSibling) {
            ...
            if (isOrContainsHeading(prevSibling))
                continue;
        ...
        for (const el of shortElements) {
            ...
            if (el.matches('a[href]') && el.parentElement && el.parentElement !== mainContent) {
                ...
                if (parentText !== text) {
                    ...
                    if (!firstHeading)
                        continue;
                    if (!(el.compareDocumentPosition(firstHeading) & 4))
                        continue;
                }
            }
    ...
    if (lastChild && ['SECTION', 'DIV', 'ASIDE'].includes(lastChild.tagName)) {
        ...
        if (paras.length >= 2 && !hasNonPara) {
            const allLinkDense = paras.every(p => {
                ...
                for (const link of links)
                    linkTextLen += (link.textContent?.trim() || '').length;
                if (linkTextLen / (text.length || 1) <= 0.6)
                    return false;
                let nonLinkText = text;
                for (const link of links)
                    nonLinkText = nonLinkText.split(link.textContent?.trim() || '').join('');
            ...
            if (allLinkDense) {
                if (debug && debugRemovals) {
                    debugRemovals.push({
                        ...
                        text: (0, utils_1.textPreview)(lastChild)
                    });
                }
                lastChild.remove();
            }
    ...
    if (totalWords > 300) {
        ...
        if (trailingEls.length >= 1 && trailingWords < totalWords * 0.15) {
            ...
            for (const el of trailingEls) {
                if (el.tagName === 'P' && (0, utils_1.countWords)(el.textContent || '') > 5) {
                    proseParagraphs++;
                }
            }
    ...
    for (const heading of mainContent.querySelectorAll('h2, h3, h4, h5, h6')) {
        ...
        if (!isCta && !RELATED_HEADING_PATTERN.test(headingText))
            continue;
        // Must appear after substantial content
        if (contentText.indexOf(headingText) < 500)
            continue;
        ...
        if (hasFollowingProse(target)) {
            if (target === heading)
                continue;
            if (debug && debugRemovals) {
                debugRemovals.push({ step: 'removeByContentPattern', reason: 'inline related content section', text: (0, utils_1.textPreview)(target) });
            }
            target.remove();
            continue;
        }
        if (target === heading) {
            ...
            if (debug && debugRemovals) {
                debugRemovals.push({ step: 'removeByContentPattern', reason: 'related content section', text: (0, utils_1.textPreview)(target) });
            }
            removeTrailingWithCascade(target, mainContent, debug, debugRemovals);
        }
        break;
    ...
    for (const el of mainContent.querySelectorAll('p')) {
        if (!el.parentNode)
            continue;
        const text = el.textContent?.trim() || '';
        if (!RELATED_INTRO_PATTERN.test(text))
            continue;
        if ((0, utils_1.countWords)(text) > 20)
            continue;
        if (el.querySelector(constants_1.CONTENT_ELEMENT_SELECTOR))
            continue;
        if (debug && debugRemovals) {
            debugRemovals.push({ step: 'removeByContentPattern', reason: 'related content intro', text: (0, utils_1.textPreview)(el) });
        }
        el.remove();
    }
    ...
    const contentWordCount = (0, utils_1.countWords)(contentText);
    for (const el of mainContent.querySelectorAll('div, ul, ol')) {
        ...
        if (el.querySelectorAll('img, picture').length < 2)
            continue;
        ...
        const cardCount = children.filter(c => c.querySelector('img, picture') && (c.querySelector('h2, h3, h4') || c.querySelector('a[href]'))).length;
        if (cardCount < 2 || cardCount < children.length * 0.7)
            continue;
        ...
        const gridWords = (0, utils_1.countWords)(el.textContent || '');
        if (contentWordCount > 0 && gridWords / contentWordCount > 0.3)
            continue;
        ...
        const targetWords = (0, utils_1.countWords)(target.textContent || '');
        if (targetWords > gridWords * 2 + 15)
            continue;
        ...
        removeThinPrecedingSection(target, debug, debugRemovals);
        if (debug && debugRemovals) {
            debugRemovals.push({ step: 'removeByContentPattern', reason: 'related post cards', text: (0, utils_1.textPreview)(target) });
        }
        removeTrailingSiblings(target, true, debug, debugRemovals);
        break;
    }
    ...
    for (const el of mainContent.querySelectorAll('div, section, aside')) {
        ...
        const elWords = (0, utils_1.countWords)(el.textContent?.trim() || '');
        let target = el;
        while (target.parentElement && target.parentElement !== mainContent) {
            const parentWords = (0, utils_1.countWords)(target.parentElement.textContent?.trim() || '');
            if (parentWords > elWords * 2 + 15)
                break;
            target = target.parentElement;
        }
        if (debug && debugRemovals) {
            debugRemovals.push({ step: 'removeByContentPattern', reason: 'newsletter signup', text: (0, utils_1.textPreview)(target) });
        }
        target.remove();
        break;
    }
    ...
    for (const el of mainContent.querySelectorAll('ul')) {
        ...
        if (!isNewsletterElement(el, 30))
            continue;
        if (debug && debugRemovals) {
            debugRemovals.push({ step: 'removeByContentPattern', reason: 'newsletter signup list', text: (0, utils_1.textPreview)(el) });
        }
        el.remove();
        break;
    }
    ...
    for (const el of mainContent.querySelectorAll('div, section')) {
        ...
        const target = walkUpIsolated(el, mainContent);
        if (debug && debugRemovals) {
            debugRemovals.push({ step: 'removeByContentPattern', reason: 'author contact block', text: (0, utils_1.textPreview)(target) });
        }
        target.remove();
        break;
    }
    ...
    for (const el of mainContent.querySelectorAll('p, span, div')) {
        ...
        if (!SHARE_AUTHOR_LABEL.test(elText))
            continue;
        let container = el;
        while (container.parentElement && container.parentElement !== mainContent) {
            const parent = container.parentElement;
            if ((0, utils_1.countWords)(parent.textContent?.trim() || '') > 15)
                break;
            container = parent;
        }
        if (container.querySelector(CONTENT_ELEMENT_NO_IMG_SELECTOR))
            continue;
        if (debug && debugRemovals) {
            debugRemovals.push({ step: 'removeByContentPattern', reason: 'author/share widget', text: (0, utils_1.textPreview)(container) });
        }
        container.remove();
    }
    ...
    for (const el of mainContent.querySelectorAll('a, p, div, span')) {
        if (!el.parentNode)
            continue;
        const text = el.textContent?.trim() || '';
        if (!SOCIAL_COUNTER_PATTERN.test(text))
            continue;
        ...
        const target = walkUpToWrapper(el, text, mainContent);
        if (debug && debugRemovals) {
            debugRemovals.push({ step: 'removeByContentPattern', reason: 'social engagement counter', text: (0, utils_1.textPreview)(target) });
        }
        target.remove();
    }
    ...
    for (const el of mainContent.querySelectorAll('div')) {
        ...
        if (linkTextLen / (text.length || 1) < 0.8)
            continue;
        if (debug && debugRemovals) {
            debugRemovals.push({ step: 'removeByContentPattern', reason: 'trailing tag link block', text: (0, utils_1.textPreview)(el) });
        }
        el.remove();
    }
}
//# sourceMappingURL=content-patterns.js.map