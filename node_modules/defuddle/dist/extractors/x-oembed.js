...
class XOembedExtractor extends _base_1.BaseExtractor {
    ...
    async extractOembed() {
        ...
        const handle = data.author_url
            ...
            : '';
        const contentHtml = (0, comments_1.buildContentHtml)('twitter', tweetText, '');
    ...
    buildTweetResult(data) {
        ...
        const contentHtml = (0, comments_1.buildContentHtml)('twitter', postContent, '');
     ...
     */
    codePointToUtf16Index(text, codePointIndex) {
        let utf16Index = 0;
        let cpCount = 0;
        for (const char of text) {
            if (cpCount >= codePointIndex)
                break;
     ...
     */
    adjustFacetIndicesToUtf16(text, facets) {
        if (facets.length === 0)
            return facets;
    ...
    renderArticle(blocks, entityMap, coverMedia, mediaEntities) {
        const parts = [];
    ...
    renderBlock(block, entityMap, mediaEntities) {
        switch (block.type) {
            case 'unstyled': {
                if (!block.text.trim())
                    return '';
    ...
    renderAtomicBlock(block, entityMap, mediaEntities) {
            ...
            case 'MEDIA': {
                ...
                for (const item of mediaItems) {
                    ...
                    if (info.__typename === 'ApiImage' && info.original_img_url) {
                        images.push(`<img src="${(0, dom_1.escapeHtml)(info.original_img_url)}" alt="${caption ? (0, dom_1.escapeHtml)(caption) : ''}">`);
                    }
                        ...
                        if (videoUrl) {
                            images.push(`<video src="${(0, dom_1.escapeHtml)(videoUrl)}" poster="${(0, dom_1.escapeHtml)(previewUrl)}" controls></video>`);
                        }
                        else {
                            images.push(`<img src="${(0, dom_1.escapeHtml)(previewUrl)}" alt="${caption ? (0, dom_1.escapeHtml)(caption) : ''}">`);
                        }
                    }
                }
                if (images.length > 0 && caption) {
                    return `<figure>${images.join('\n')}<figcaption>${(0, dom_1.escapeHtml)(caption)}</figcaption></figure>`;
                }
