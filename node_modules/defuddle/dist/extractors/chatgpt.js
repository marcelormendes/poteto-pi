...
class ChatGPTExtractor extends _conversation_1.ConversationExtractor {
        ...
        this.turns.forEach((turn) => {
            ...
            const contentHtmlParts = messageEls.flatMap(el => {
                ...
                return contentElements.length > 0
                    ? contentElements.map(contentEl => (0, dom_1.serializeHTML)(contentEl))
                    : [(0, dom_1.serializeHTML)(el)];
            });
            let messageContent = (contentHtmlParts.length > 0 ? contentHtmlParts : [(0, dom_1.serializeHTML)(turn)]).join('\n');
            ...
            messageContent = messageContent.replace(citationPattern, (match, zws, spanStructure, url) => {
                ...
                if (footnoteIndex === -1) {
                    ...
                    this.footnotes.push({
                        url,
                        text: `<a href="${(0, dom_1.escapeHtml)(url)}">${(0, dom_1.escapeHtml)(domain)}</a>${(0, dom_1.escapeHtml)(fragmentText)}`
                    });
                }
                else {
            ...
            cleanupDiv.appendChild((0, dom_1.parseHTML)(this.document, messageContent));
            cleanupDiv.querySelectorAll('span[data-state="closed"]').forEach(el => el.remove());
            messageContent = (0, dom_1.serializeHTML)(cleanupDiv);
    ...
    getMessageContentElements(messageEl) {
