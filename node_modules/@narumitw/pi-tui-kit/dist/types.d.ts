...
export type MenuActionResult<ScreenId extends string> = MenuTransition<ScreenId> | {
    kind: "rejected";
    error?: unknown;
} | undefined;
...
export type MenuActionHandler<State, ScreenId extends string, Context extends MenuContext = ExtensionCommandContext> = (context: MenuActionContext<State, Context>) => MenuActionResult<ScreenId> | Promise<MenuActionResult<ScreenId>>;
...
export type ActionMenuItem<ScreenId extends string, ActionId extends string> = (ActionMenuItemBase & {
    to: ScreenId;
    action?: never;
    close?: never;
}) | (ActionMenuItemBase & {
...
export declare const MAX_REVIEW_VIEWPORT_SIZE = 50;
...
export type MenuScreen<ScreenId extends string, ActionId extends string> = ActionsScreen<ScreenId, ActionId> | DetailScreen | BrowseScreen | ChoiceScreen<ActionId> | SettingsScreen<ActionId> | InputScreen<ActionId> | ReviewScreen<ActionId> | MultiSelectScreen<ScreenId, ActionId>;
export interface MenuScreenContext<State> {
    state: State;
}
export type MenuScreenFactory<State, ScreenId extends string, ActionId extends string> = (context: MenuScreenContext<State>) => MenuScreen<ScreenId, ActionId>;
export interface MenuDefinition<State, ScreenId extends string, ActionId extends string, Context extends MenuContext = ExtensionCommandContext> {
    start: ScreenId;
