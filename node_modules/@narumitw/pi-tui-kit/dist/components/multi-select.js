...
import { actionMenuItemPresentation, actionMenuUnavailableDescription, handleSearchInput, renderFrame, safeMenuText, } from "./rendering.js";
export function createMultiSelectComponent(options) {
    ...
    const applyFilter = () => {
        ...
        const filteredRows = fuzzyFilter(searchableRows, searchInput.getValue(), (candidate) => candidate.text).map((candidate) => candidate.row);
        ...
        render(width) {
            ...
            const rowContent = visibleRows.map((row, offset) => {
                ...
                const label = row.kind === "action"
                    ...
                    : `${prefix}${row.item.disabled ? "[-]" : selected.get(row.item.id) ? "[x]" : "[ ]"} ${safeMenuText(row.item.label)}${row.item.disabled ? " (unavailable)" : ""}`;
                const boundedLabel = row.kind === "action"
            ...
            const descriptions = row
                ...
                : [];
            const descriptionRows = descriptions.flatMap((description) => wrapTextWithAnsi(options.theme.fg("dim", `  ${safeMenuText(description)}`), safeWidth));
