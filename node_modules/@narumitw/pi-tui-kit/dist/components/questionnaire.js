...
import { Editor, Key, matchesKey, sliceByColumn, truncateToWidth, visibleWidth, } from "@earendil-works/pi-tui";
...
export class QuestionnaireComponent {
    ...
    renderQuestion(width) {
        ...
        question.options.forEach((option, index) => {
            ...
            const line = selected
                ...
                : `${this.options.theme.fg("text", label)}${chosen ? this.options.theme.fg("success", " ✓") : ""}${description ? this.options.theme.fg("muted", description) : ""}`;
        ...
        if (this.editorKind) {
            lines.push("", this.options.theme.fg("accent", sanitizeQuestionnaireText(this.editorKind === "answer"
                ? this.options.labels.customAnswer
                : this.options.labels.optionalNote)), ...this.editor.render(width));
        }
        return lines;
    }
    renderCustomOption(question) {
        ...
        return [
            selected
                ...
                : `${this.options.theme.fg("text", label)}${chosen ? this.options.theme.fg("success", " ✓") : ""}`,
        ];
    }
    renderReview(width) {
        ...
        this.options.questions.forEach((question, index) => {
            ...
            const summary = ` — ${inlineSummary(answer?.answer ?? "Unanswered")}${answer?.note ? ` · ${noteLabel}: ${inlineSummary(answer.note)}` : ""}`;
    ...
    submitPage() {
        if (this.isReviewPage()) {
            ...
            this.finish({
                kind: "submitted",
                answers: this.answers.filter((answer) => answer !== undefined),
            });
            return;
        }
        const question = this.options.questions[this.page];
    ...
    submitEditor(value) {
        ...
        const answer = this.answers[index];
        if (answer)
            answer.note = value.trim() ? value : undefined;
        this.editor.setText("");
        this.editorKind = undefined;
        this.editor.focused = false;
    ...
    advance() {
        if (!this.hasReviewPage()) {
            const answers = this.answers.filter((answer) => answer !== undefined);
...
class RawPreservingEditor {
    editor;
    rawByMarker = new Map();
    markerCodePoint = 0xe000;
    pasteBuffer;
    ...
    nextMarker(forbidden) {
        for (;;) {
            if (this.markerCodePoint === 0xf900)
                this.markerCodePoint = 0xf0000;
            if (this.markerCodePoint === 0xffffe)
                this.markerCodePoint = 0x100000;
