export function createRpcHarness(steps) {
    ...
    let waitIndex = 0;
    let pendingCalls = 0;
    const input = async (title, placeholder, options) => {
        const step = consumeStep("input", { title, placeholder }, options);
        if (options?.signal?.aborted)
            return undefined;
        if (step.waitForAbort)
            return waitForAbort(options?.signal, "input");
    ...
    const select = async (title, options, dialogOptions) => {
        ...
        if (dialogOptions?.signal?.aborted)
            return undefined;
        if (step.waitForAbort)
            return waitForAbort(dialogOptions?.signal, "select");
        if (step.response !== undefined && !options.includes(step.response)) {
            throw new Error(`Scripted select response ${JSON.stringify(step.response)} is not one of the offered options`);
        }
        return step.response;
    };
    const custom = async () => {
    ...
    function consumeStep(kind, actual, options) {
        const step = script[scriptIndex];
        if (!step)
            throw new Error(`Unexpected ${kind} RPC call: script is exhausted`);
        if (step.kind !== kind) {
            throw new Error(`Expected ${step.kind} RPC call but received ${kind}`);
        }
        if (step.title !== undefined && step.title !== actual.title) {
            throw new Error(`Expected ${kind} title ${JSON.stringify(step.title)} but received ${JSON.stringify(actual.title)}`);
        }
        if (kind === "input" && step.kind === "input") {
            if (step.placeholder !== undefined && step.placeholder !== actual.placeholder) {
                throw new Error(`Expected input placeholder ${JSON.stringify(step.placeholder)} but received ${JSON.stringify(actual.placeholder)}`);
            }
        }
        else if (kind === "select" && step.kind === "select" && step.options !== undefined) {
            if (!sameStrings(step.options, actual.options ?? [])) {
                throw new Error(`Expected select options ${JSON.stringify(step.options)} but received ${JSON.stringify(actual.options ?? [])}`);
            }
        }
        scriptIndex += 1;
    ...
    async function waitForAbort(signal, kind) {
        if (!signal)
            throw new Error(`Pending ${kind} RPC step requires an AbortSignal`);
        if (signal.aborted)
            return undefined;
        pendingCalls += 1;
        let onAbort;
    ...
    return {
        ...
        assertConsumed() {
            ...
            if (remaining > 0) {
                throw new Error(`${remaining} scripted RPC step${remaining === 1 ? " remains" : "s remain"}`);
            }
            if (pendingCalls > 0)
                throw new Error(`${pendingCalls} scripted RPC call is still pending`);
        },
    };
}
