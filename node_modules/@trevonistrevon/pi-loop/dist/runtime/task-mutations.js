...
export function taskMutationRejectionMessage(id, result) {
    if (result.code === "not_found")
        return `Task #${id} not found`;
    if (result.code === "no_changes")
        return "No update fields were provided.";
    if (result.code === "claim_required")
        return `Task #${id} has a live claim; claimId is required.`;
    if (result.code === "claim_missing")
        return `Task #${id} has no live claim; claim the task before sending a heartbeat.`;
    if (result.code === "claim_mismatch")
        return "Claim token does not match the live task owner.";
    if (result.code === "claim_expired")
        return `Task #${id} claim lease expired; reclaim the task before modifying it.`;
    if (result.code === "terminal")
        return `Task #${id} is ${result.entry?.status}; terminal tasks cannot renew claims.`;
    if (result.code === "mutation_conflict")
        return `Task #${id} changed while the update was applied; refresh it and retry.`;
    return `Transition from ${result.fromStatus} to ${result.toStatus} is not allowed.`;
}
function claimRejection(entry, claimId, now) {
    if (!entry.claim)
        return undefined;
    if (entry.claim.leaseExpiresAt <= now)
        return "claim_expired";
    if (!claimId)
        return "claim_required";
    if (entry.claim.claimId !== claimId)
...
export async function claimTask(ctx, params) {
    ...
    if (!result)
        return undefined;
    if (!result.renewed)
        emitNativeTaskEvent(ctx.pi, "tasks:started", result.entry, previous.status);
    const backlog = await settle(ctx);
    return { result, backlog };
}
export function heartbeatTask(ctx, params) {
    ...
    if (!current.claim)
        return { applied: false, code: "claim_missing", entry: current };
    const rejection = claimRejection(current, params.claimId, Date.now());
    if (rejection)
        return { applied: false, code: rejection, entry: current };
    const entry = ctx.taskStore.heartbeat(params.id, params.claimId, Date.now(), params.leaseMs);
    if (!entry)
        return { applied: false, code: "mutation_conflict", entry: ctx.taskStore.get(params.id) };
    ctx.updateWidget();
    return { applied: true, entry, backlog: { created: false } };
}
export async function updateTask(ctx, params) {
    ...
    if ((subject !== undefined || description !== undefined) && entry.claim) {
        ...
        if (rejection)
            return { applied: false, code: rejection, entry };
    }
    const previousStatus = entry.status;
    if (status === entry.status && (status === "pending" || status === "in_progress")) {
        if (status === "in_progress" && entry.claim) {
            if (entry.claim.leaseExpiresAt <= Date.now())
                return { applied: false, code: "claim_expired", entry };
            if (claimId && entry.claim.claimId !== claimId)
                return { applied: false, code: "claim_mismatch", entry };
        }
        ...
        if (entry)
            emitNativeTaskEvent(ctx.pi, "tasks:started", entry, previousStatus);
    }
        ...
        if (entry)
            emitNativeTaskEvent(ctx.pi, status === "completed" ? "tasks:completed" : "tasks:closed", entry, previousStatus);
    }
    else if (status === "pending") {
        entry = ctx.taskStore.reopen(id);
        if (entry)
            emitNativeTaskEvent(ctx.pi, "tasks:reopened", entry, previousStatus);
    }
    if (!entry) {
        ...
        const lateClaimRejection = current && (status === "in_progress" || status === "completed" || status === "closed")
            ...
            : undefined;
        if (current && lateClaimRejection)
            return { applied: false, code: lateClaimRejection, entry: current };
        const transitionStillValid = current && ((status === "in_progress" && current.status === "pending")
    ...
    if (subject !== undefined || description !== undefined) {
        ...
        entry = ctx.taskStore.updateDetails(id, { subject, description }, { claimId, expectedRevision, now: Date.now() });
        if (!entry) {
            const current = ctx.taskStore.get(id);
            if (!current)
                return { applied: false, code: "not_found" };
...
export async function cleanTasks(ctx) {
