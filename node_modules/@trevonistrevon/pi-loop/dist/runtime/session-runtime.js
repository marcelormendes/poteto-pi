...
export function registerSessionRuntimeHooks(options) {
    const { pi, getLoopScope, getPiLoopEnv, getSessionGeneration, advanceSessionGeneration, recreateSessionStore, clearAllLoops, getStore, getScheduler, getTriggerSystem, setLatestCtx, setSessionId, widget, notificationRuntime, flushPendingNotifications, migrateTaskBacklogLoops, cleanupTaskBacklogLoops, adoptTaskBacklogLoops, releaseTaskBacklogWakes, clearWorkflowMonitorWaits, recoverOrchestrations, pumpOrchestrations, shutdownOrchestrations, shutdownMonitors, hasPendingTasks, cleanDoneTasks, isContextCurrent, emitLoopExpired, } = options;
    let storeUpgraded = false;
    let persistedShown = false;
    let heartbeatTimer;
    let agentStartFireCounts;
    const isCurrentGeneration = (generation) => generation === getSessionGeneration();
    ...
    async function showPersistedLoops(generation = getSessionGeneration()) {
        if (!isCurrentGeneration(generation) || persistedShown)
            return;
        persistedShown = true;
        ...
        if (!isCurrentGeneration(generation) || !isContextCurrent())
            return;
    ...
    async function pumpLoops(generation = getSessionGeneration()) {
