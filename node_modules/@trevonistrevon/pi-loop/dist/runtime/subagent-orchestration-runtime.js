...
import { RpcError } from "../rpc/cross-extension-rpc.js";
...
function normalizeLifecycle(kind, raw) {
...
function buildWorkerPrompt(entry, item) {
    return [
...
function asSpawnReply(value) {
    if (!value || typeof value !== "object")
        return undefined;
    const id = value.id;
    return typeof id === "string" && id.length > 0 ? { id } : undefined;
}
export function createSubagentOrchestrationRuntime(options) {
    const { events, getStore, getScope, getActor, getGeneration, rpcCall, emitWake, updateWidget, debug, } = options;
    ...
    const wakeQueued = new Set();
    const consumeInFlight = new Set();
    const dispatchesInFlight = new Map();
    let active = true;
    let reconcileQueued = false;
    let pumpPromise;
    function actorOwns(state) {
    ...
    function rememberEarly(event) {
        pruneEarlyEvents();
        earlyEvents.set(event.id, { event, expiresAt: now() + EARLY_EVENT_TTL_MS });
        pruneEarlyEvents();
    }
    function findActiveAgent(agentId) {
        for (const entry of getStore().list()) {
            const state = entry.orchestration;
            if (!state || !actorOwns(state))
                continue;
            if (state.work.some((item) => currentDispatch(item)?.agentId === agentId))
                return { entry, state };
        }
        return undefined;
    }
    function findSettledAgent(agentId) {
        for (const entry of getStore().list()) {
            const state = entry.orchestration;
            if (!state || !actorOwns(state))
                continue;
    ...
    function trackDispatch(loopId, promise) {
        const pending = dispatchesInFlight.get(loopId) ?? new Set();
        pending.add(promise);
        dispatchesInFlight.set(loopId, pending);
        const remove = () => {
            pending.delete(promise);
            if (pending.size === 0)
                dispatchesInFlight.delete(loopId);
        };
        void promise.then(remove, remove);
        return promise;
    }
    async function drainDispatches(loopId) {
    ...
    function recordConsume(loopId, agentId, consumed) {
        if (!isContextCurrent())
            return;
    ...
    function canConsume(loopId, agentId) {
    ...
    function consumeSettled(loopId, agentId) {
        if (consumeInFlight.has(agentId) || !canConsume(loopId, agentId))
            return;
        ...
        void call.then(() => recordConsume(loopId, agentId, true), () => recordConsume(loopId, agentId, false)).finally(() => {
            consumeInFlight.delete(agentId);
        });
    }
    function handleLifecycle(event, rememberUnknown = true) {
        if (!active || !isContextCurrent())
            return false;
    ...
    function isAgentBound(agentId) {
        return getStore().list().some((entry) => entry.orchestration?.work.some((item) => item.dispatches.some((dispatch) => dispatch.agentId === agentId)));
    }
    async function stopUnboundSpawn(agentId) {
        if (isAgentBound(agentId))
            return;
        try {
    ...
    async function dispatchWork(entry, item) {
        if (!isContextCurrent())
            return;
        const state = entry.orchestration;
        if (!state || !actorOwns(state) || state.status !== "active")
            return;
        ...
        if (!reservedEntry || !reserved || !reservedItem)
            return;
        try {
            ...
            if (!reply)
                throw new Error("Invalid subagent spawn reply");
            if (!isContextCurrent()) {
    ...
    function retryPendingConsumes(entry) {
        const state = entry.orchestration;
        if (!state || !actorOwns(state))
            return;
    ...
    function providerOwnsTerminalWake(state) {
        const counts = getOrchestrationCounts(state);
        if (counts.pending > 0 || counts.active > 0 || counts.uncertain > 0)
            return false;
    ...
    function emitPendingWakes() {
        for (const entry of getStore().list()) {
            const state = entry.orchestration;
            const wake = state?.pendingWake;
            if (!state || !wake || !actorOwns(state))
                continue;
            if (getOrchestrationCounts(state).active > 0)
                continue;
            const key = `${entry.id}:${wake.sequence}`;
            if (wakeQueued.has(key))
                continue;
    ...
    async function runPump() {
        if (!active || !isContextCurrent() || getScope() !== "session" || getPiLoopEnv())
            return;
        const actor = getActor();
        if (!actor || actor.generation !== getGeneration())
            return;
        for (const listed of getStore().list()) {
            ...
            if (entry.status !== "active" && !recoveringWake)
                continue;
            if (!actorOwns(state)) {
            ...
            if (entry.status !== "active" || state.status !== "active")
                continue;
            while (true) {
                const freshEntry = getStore().get(entry.id);
                const fresh = freshEntry?.orchestration;
                if (!freshEntry || !fresh || freshEntry.status !== "active" || !actorOwns(fresh) || fresh.status !== "active")
                    break;
                if (getOrchestrationCounts(fresh).active >= fresh.concurrency)
                    break;
    ...
    async function stopDispatch(loopId, workId, dispatch) {
        if (!dispatch.agentId)
            return;
        let stopped = false;
        ...
        if (stopped && settled.applied)
            consumeSettled(loopId, dispatch.agentId);
    }
    async function shutdown() {
        ...
        for (const entry of getStore().list()) {
            const state = entry.orchestration;
            if (!state || state.owner.runtimeId !== getActor()?.runtimeId)
                continue;
            for (const item of state.work) {
                const dispatch = currentDispatch(item);
                if (dispatch?.agentId)
                    stops.push(stopDispatch(entry.id, item.id, { ...dispatch }));
                else if (dispatch) {
                    const fresh = getStore().get(entry.id)?.orchestration;
                    if (fresh) {
                        getStore().mutateOrchestration(entry.id, {
    ...
    async function stopCancelledAgent(loopId, agentId) {
        try {
            await rpcCall(STOP_RPC, { agentId }, STOP_TIMEOUT_MS);
    ...
    async function cancel(id, action) {
        ...
        if (action === "delete")
            ...
            getStore().pause(id, "administrative", "orchestration cancelled by operator");
    ...
    function acknowledgeWake(id, sequence) {
        const entry = getStore().get(id);
        const state = entry?.orchestration;
        if (!state || !actorOwns(state))
            return false;
