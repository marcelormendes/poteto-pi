...
export const AUTO_TASK_WORKER_LEGACY_PROMPTS = [
...
export function isAutoTaskWorkerPrompt(prompt) {
...
export function createTaskBacklogRuntime(options) {
    const { getLoops, deleteLoop, updateLoopWorker, recordDeletionTombstone, removeTrigger, updateWidget, hasPendingTasks, adoptLoop, triggerHasEventSource, emitLoopAutodeleted, emitTaskBacklogEmpty, debug, } = options;
    ...
    function migrateAutoTaskWorkerPrompts() {
        let migrated = 0;
        for (const entry of getLoops()) {
            if (!isAutoTaskWorkerPrompt(entry.prompt))
                continue;
            if (!triggerHasEventSource(entry.trigger, "tasks:created"))
                continue;
            if (entry.prompt === AUTO_TASK_WORKER_PROMPT && entry.taskBacklog)
                continue;
            if (updateLoopWorker(entry.id, AUTO_TASK_WORKER_PROMPT))
                migrated++;
        }
        return migrated;
    }
    ...
    async function adoptTaskBacklogLoops(baselineFireCounts, isCurrent) {
    ...
    async function cleanupTaskBacklogLoops(isCurrent) {
        if (isCurrent && !isCurrent())
            return 0;
