...
import { atMaxFires, reduceLoopState } from "./loop-reducer.js";
import { applyOrchestrationEvent, validateOrchestrationDefinition, validatePersistedOrchestration } from "./orchestration-reducer.js";
import { ReducerBackedStore } from "./reducer-backed-store.js";
import { validateWorkflowDefinition } from "./workflow-definition.js";
import { atWorkflowStateFireLimit, isTerminalWorkflowRun, transitionWorkflowRun, validateWorkflowAdmissionRecord } from "./workflow-reducer.js";
import { validatePersistedWorkflowRevision } from "./workflow-revision.js";
 ...
 */
function normalizeWorkflowRunState(workflow) {
    ...
    if (hasRevision !== hasHistory)
        throw new Error("Malformed workflow revision metadata: revision and history must appear together");
    ...
    for (const [index, revision] of revisionHistory.entries()) {
        ...
        if (revisionError)
            throw new Error(`Malformed workflow revision metadata: ${revisionError}`);
    }
    ...
    const needsExecution = Boolean(state?.task
...
const LOOP_PAUSE_KINDS = new Set([
    "administrative",
    "controller_limit",
    "semantic_terminal",
    "orchestration_settlement",
]);
function normalizePauseRecord(entry) {
...
function workflowTransitionPausePolicy(entry, input) {
    if (entry.status !== "paused")
        return { resumeAfterStateLimitExit: false };
    if (!entry.workflow)
        return { error: `Loop #${entry.id} is not a workflow loop`, resumeAfterStateLimitExit: false };
    ...
    if (workflow.definition.states[target]?.terminal)
        return { resumeAfterStateLimitExit: false };
...
export class LoopStore extends ReducerBackedStore {
    ...
    create(trigger, prompt, opts) {
        return this.withLock(() => {
            ...
            if (opts.orchestration) {
                if (trigger.type !== "dynamic")
                    throw new Error("Orchestration loops require a dynamic trigger.");
                if (opts.workflow)
                    throw new Error("A loop cannot own both workflow and orchestration state.");
                const validationError = validateOrchestrationDefinition(opts.orchestration.definition);
                if (validationError)
                    throw new Error(`Invalid orchestration: ${validationError}`);
            }
            const now = Date.now();
    ...
    mutateOrchestration(id, event) {
        return this.withLock(() => {
            const entry = this.entries.get(id);
            if (!entry)
                return { applied: false, reason: "loop_not_found" };
            if (!entry.orchestration)
                return { applied: false, reason: "not_orchestration" };
            const result = applyOrchestrationEvent(entry.orchestration, event);
            if (!result.applied)
                return { entry, applied: false, reason: result.reason };
            this.applyReducerEvent({
    ...
    pause(id, kind = "administrative", reason) {
        return this.withLock(() => {
            const entry = this.entries.get(id);
            if (!entry)
                return undefined;
    ...
    fire(id, origin = "scheduler") {
        return this.withLock(() => {
            const entry = this.entries.get(id);
            if (entry?.status !== "active" || isTerminalWorkflowRun(entry?.workflow))
                return undefined;
            this.applyReducerEvent({
                type: "LOOP_FIRED",
                at: Date.now(),
    ...
    continueDynamic(id, fields, expected) {
        return this.withLock(() => {
            const entry = this.entries.get(id);
            if (!entry || entry.trigger.type !== "dynamic" || !entry.dynamic || entry.workflow)
                return undefined;
            if (expected && (entry.status !== expected.status
    ...
    stopDynamic(id, status, expected) {
        return this.withLock(() => {
            const entry = this.entries.get(id);
            if (!entry || entry.trigger.type !== "dynamic" || !entry.dynamic || entry.workflow
    ...
    transitionWorkflow(id, input, expected) {
        return this.withLock(() => {
            const entry = this.entries.get(id);
            if (!entry)
            ...
            if (expected && (entry.workflow.currentState !== expected.currentState
            ...
            if (pausePolicy.error)
                return { applied: false, error: pausePolicy.error };
            ...
            if (result.terminal) {
                const terminalEntry = {
                    ...
                    pause: result.terminal === "paused"
                        ? { kind: "semantic_terminal", at: result.run.stateEnteredAt }
                        : undefined,
    ...
    claimWorkflowExecution(id, actor, leaseSeconds = 1800) {
        const leaseMs = Math.min(Math.max(leaseSeconds, 60), 3600) * 1000;
        return this.withLock(() => {
            const entry = this.entries.get(id);
            if (!entry)
                return { claimed: false, error: `Loop #${id} not found` };
    ...
    attachWorkflowMonitor(id, monitorId, expected) {
        return this.withLock(() => {
            const entry = this.entries.get(id);
            if (!entry?.workflow || entry.status !== "active" || entry.workflow.waitingMonitor)
                return undefined;
    ...
    settleWorkflowMonitorWait(id, expected, now = Date.now()) {
        return this.withLock(() => {
            const expired = this.expireEntryUnlocked(id, now);
            if (expired)
                return { kind: "expired", ...expired };
            const entry = this.entries.get(id);
            ...
            if (!wait
                ...
                || wait.attachedAt !== expected.attachedAt)
                return { kind: "stale" };
            this.applyReducerEvent({
    ...
    completeWorkflowMonitorWait(id, expected) {
        return this.withLock(() => {
            const entry = this.entries.get(id);
            const wait = entry?.workflow?.waitingMonitor;
            if (!wait
    ...
    expireEventLoopEntries(sessionStartedAt) {
        return this.withLock(() => {
            const expired = [];
            for (const [id, entry] of [...this.entries.entries()]) {
                ...
                if (entry.taskBacklog && eventSource === "tasks:created")
                    continue;
