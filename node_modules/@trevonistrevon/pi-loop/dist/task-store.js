...
export class TaskStore extends ReducerBackedStore {
    ...
    claim(id, input) {
        ...
        return this.withLock(() => {
            ...
            if (!current || current.status === "completed" || current.status === "closed")
                return undefined;
            ...
            if (existingIsLive && !sameLiveOwner)
                return undefined;
            ...
            const claim = sameLiveOwner && existing
                ? {
                    ...existing,
                    heartbeatAt: now,
    ...
    heartbeat(id, claimId, now = Date.now(), leaseMs = 30 * 60 * 1000) {
        if (!claimId || !Number.isSafeInteger(leaseMs) || leaseMs <= 0 || leaseMs > MAX_TASK_LEASE_MS)
            return undefined;
        return this.withLock(() => {
            const current = this.entries.get(id);
            if (!current?.claim
    ...
    start(id) {
        return this.withLock(() => {
            ...
            if (!entry || entry.status === "completed" || entry.status === "closed" || entry.claim)
                return undefined;
            this.applyReducerEvent({
                type: "TASK_STARTED",
                at: Date.now(),
    ...
    complete(id, claimId, now = Date.now()) {
        return this.withLock(() => {
            const entry = this.entries.get(id);
            if (!entry
    ...
    reopen(id) {
        return this.withLock(() => {
            const entry = this.entries.get(id);
            if (!entry || (entry.status !== "completed" && entry.status !== "closed"))
                return undefined;
            this.applyReducerEvent({
                type: "TASK_REOPENED",
                at: Date.now(),
    ...
    updateDetails(id, fields, options = {}) {
        return this.withLock(() => {
            const entry = this.entries.get(id);
            if (!entry)
                return undefined;
            const now = options.now ?? Date.now();
            if (options.expectedRevision !== undefined && entry.revision !== options.expectedRevision)
                return undefined;
            if (entry.claim && (entry.claim.leaseExpiresAt <= now
                || !options.claimId
    ...
    delete(id, claimId, now = Date.now()) {
        return this.withLock(() => {
            const entry = this.entries.get(id);
            if (!entry
                || (entry.claim && (entry.claim.claimId !== claimId || entry.claim.leaseExpiresAt <= now))) {
