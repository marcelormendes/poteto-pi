...
function validateWorkflowKey(key, kind) {
    if (WORKFLOW_KEY_PATTERN.test(key) && !RESERVED_WORKFLOW_KEYS.has(key))
        return undefined;
    return `Workflow ${kind} key "${key}" must match ${WORKFLOW_KEY_PATTERN.source} and cannot be reserved`;
}
function validateTask(stateId, task) {
    if (task === undefined)
        return undefined;
    if (!task || typeof task !== "object" || Array.isArray(task))
        return `State "${stateId}" task must be an object`;
    if (typeof task.subject !== "string" || !task.subject.trim())
        return `State "${stateId}" task requires a subject`;
    if (typeof task.description !== "string" || !task.description.trim()) {
        return `State "${stateId}" task requires a description`;
    }
    return undefined;
}
function validateLoop(stateId, loop) {
    if (loop === undefined)
        return undefined;
...
function validateTransitions(definition, stateId, state, allowUnboundedSelfLoops) {
...
function validateTerminal(stateId, state) {
    ...
    if (state.task)
        return `Terminal state "${stateId}" cannot declare task work`;
    if (state.loop)
        return `Terminal state "${stateId}" cannot declare a loop policy`;
    return undefined;
}
function validateState(definition, stateId, state, allowUnboundedSelfLoops) {
