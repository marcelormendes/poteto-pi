...
import { admitWorkflowTransition } from "../workflow-admission.js";
import { validateWorkflowDefinition } from "../workflow-definition.js";
import { getActiveWorkflowStateLoop, getWorkflowOutcomeAvailability } from "../workflow-reducer.js";
import { WorkflowRevisionChangeSchema } from "../workflow-schema.js";
import { textResult } from "./tool-result.js";
function revisionRecovery(code) {
    switch (code) {
...
const WORKFLOW_DEFINITION_EXAMPLE = '{"version":1,"initialState":"collect","states":{"collect":{"prompt":"Collect evidence.","on":{"ready":"publish"}},"publish":{"prompt":"Publish the result.","terminal":"completed"}}}';
...
function workflowDefaultMaxFires(definition) {
...
function stateLoopStartsImmediately(entry) {
...
function stateShouldWakeImmediately(entry) {
    return !entry.workflow || !getActiveWorkflowStateLoop(entry.workflow) || stateLoopStartsImmediately(entry);
}
function workflowWakeDescription(entry) {
    if (entry.status === "paused")
        return "no next wake is scheduled while the controller is paused.";
    if (entry.workflow && getActiveWorkflowStateLoop(entry.workflow))
        return "scheduled from the active state cadence.";
...
export function formatWorkflowSummary(entry, heading, failure, now = Date.now()) {
...
export function registerWorkflowTools(options) {
    const { pi, getStore, getTriggerSystem, getActor, getAdmissionContextDigest, getAdmissionProviders, updateWidget, onDynamicLoopActivated, } = options;
    pi.registerTool({
        name: "WorkflowCreate",
        ...
        async execute(_toolCallId, params) {
            ...
            if (stateShouldWakeImmediately(entry))
                onDynamicLoopActivated?.(entry);
            const current = store.get(entry.id) ?? entry;
            if (current.status !== "active")
                getTriggerSystem().remove(current.id);
            updateWidget();
            const wake = workflowWakeDescription(current);
            return textResult(`${formatWorkflowSummary(current, `Workflow #${current.id} created — ${current.status}`)}\nWake: ${wake}`, workflowDisplayDetails({
                entry: current,
                action: "create",
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params) {
            ...
            if (summary.addedStates.length > 0)
                lines.push(`Added states: ${summary.addedStates.join(", ")}`);
            if (summary.revisedStates.length > 0)
                lines.push(`Revised state work: ${summary.revisedStates.join(", ")}`);
            if (summary.reissuedStates.length > 0)
                lines.push(`Reissued active state: ${summary.reissuedStates.join(", ")}`);
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params) {
            ...
            if (result.terminal === "paused") {
                return textResult(`Workflow #${entry.id} paused\nFinal state: ${entry.workflow?.currentState ?? "?"}`, workflowDisplayDetails({
                    entry,
                    action: "transition",
                    tone: "warning",
            ...
            return textResult(`Workflow #${current.id} advanced: ${transition}\n${formatWorkflowSummary(current, `Workflow #${current.id} — ${current.status}`)}`, workflowDisplayDetails({
                entry: current,
                action: "transition",
                tone: current.status === "paused" ? "warning" : "success",
