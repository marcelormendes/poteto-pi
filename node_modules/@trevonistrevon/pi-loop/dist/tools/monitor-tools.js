...
function formatActivity(monitor) {
    const now = Date.now();
    const lastActivityAt = Math.max(monitor.startedAt, monitor.lastActivityAt ?? 0, monitor.lastOutputAt ?? 0, monitor.progress?.updatedAt ?? 0);
    const silence = now - lastActivityAt;
    if (silence >= 60000)
        return `quiet ${formatRemaining(silence)}`;
...
export function registerMonitorTools(options) {
    const { pi, getStore, getMonitorManager, getTriggerSystem, updateWidget, handleMonitorDoneLoop, handleWorkflowMonitorWait, } = options;
    pi.registerTool({ name: "MonitorCreate", label: "MonitorCreate",
        ...
        execute(_toolCallId, params) {
            ...
            if (params.onDone) {
                ...
                const timeoutTrigger = { type: "event", source: "monitor:timeout", filter: JSON.stringify({ monitorId: entry.id }) };
                const timeoutLoop = store.create(timeoutTrigger, `Monitor #${entry.id} became stale after ${entry.timeout}ms without output or progress. Inspect MonitorList, report the failure, and decide whether to retry or recover the command.`, { recurring: false });
    ...
    pi.registerTool({
        ...
        execute() {
            ...
            for (const m of monitors) {
                ...
                if (m.stopReason)
                    line += ` reason=${m.stopReason}`;
                if (m.progress)
                    line += ` · ${formatProgress(m.progress)}`;
