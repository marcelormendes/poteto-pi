...
function validateCreate(params) {
    if (!params.goal.trim() || params.goal.length > MAX_GOAL_CHARS)
        return `goal must contain 1-${MAX_GOAL_CHARS} characters`;
    if (params.work.length < 1 || params.work.length > MAX_WORK_ITEMS)
        return `Orchestration requires between 1 and ${MAX_WORK_ITEMS} work items.`;
    const concurrency = params.concurrency ?? 3;
    if (!Number.isInteger(concurrency) || concurrency < 1 || concurrency > MAX_CONCURRENCY)
        return `concurrency must be between 1 and ${MAX_CONCURRENCY}`;
    const maxAttempts = params.maxAttempts ?? 1;
    if (!Number.isInteger(maxAttempts) || maxAttempts < 1 || maxAttempts > MAX_ATTEMPTS)
        return `maxAttempts must be between 1 and ${MAX_ATTEMPTS}`;
    ...
    for (const [index, item] of params.work.entries()) {
        if (!item.prompt.trim())
            return `work item ${index + 1} prompt must not be empty`;
        if (item.prompt.length > MAX_PROMPT_CHARS)
            return `work prompt exceeds ${MAX_PROMPT_CHARS} characters at item ${index + 1}`;
        if ((item.agentType ?? params.agentType ?? "general-purpose").length > 128)
            return `agentType exceeds 128 characters at item ${index + 1}`;
    }
...
function definitionFrom(params) {
    return {
...
function formatWork(item) {
    ...
    if (dispatch?.agentId)
        line += ` · agent=${dispatch.agentId}`;
    if (item.attemptCount > 0)
        line += ` · attempts=${item.attemptCount}`;
    return line;
}
function formatController(entry, state) {
...
function formatWorkDetails(entry, state, item) {
    const lines = [
    ...
    for (const dispatch of item.dispatches) {
        ...
        if (dispatch.result)
            lines.push(`Result: ${dispatch.result}`);
        if (dispatch.error)
...
export function registerSubagentOrchestrationTools(options) {
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params) {
            if (getScope() !== "session")
                return errorResult("Orchestration requires the default file-backed session scope; memory and project scopes are unsupported.");
            if (getPiLoopEnv() !== undefined)
                return errorResult("Orchestration is unavailable when PI_LOOP overrides storage; unset PI_LOOP and use session scope.");
            const actor = getActor();
            if (!actor)
                return errorResult("Orchestration requires an active session runtime.");
            ...
            if (protocolVersion(probe) !== 2)
                return errorResult("Orchestration requires pi-subagents protocol v2; no compatible provider replied.");
