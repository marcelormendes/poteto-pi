...
import { claimTask, createTask, deleteTask, heartbeatTask, taskMutationRejectionMessage, updateTask, } from "../runtime/task-mutations.js";
...
Use when TaskList's excerpt is truncated or you need the complete goal-state and next-step text before starting a chained task. Read-only — does not change task state.`,
        ...
        execute(_toolCallId, params) {
            ...
            if (t.reopenedAt)
                lines.push(`Reopened: ${new Date(t.reopenedAt).toISOString()}`);
            if (t.closedAt)
                lines.push(`Closed: ${new Date(t.closedAt).toISOString()}`);
