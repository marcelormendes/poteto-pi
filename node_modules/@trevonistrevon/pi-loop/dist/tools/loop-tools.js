...
function parseDelayMs(input) {
    ...
    if (!Number.isSafeInteger(delayMs) || delayMs <= 0 || delayMs > 7 * 24 * 60 * 60 * 1000)
        return undefined;
    return delayMs;
}
...
function formatDynamicUpdateResult(id, iteration, nextWakeAt, resumed) {
...
function stopDynamicLoop(params, entry, store, triggerSystem) {
...
export function registerLoopTools(options) {
    const { pi, getStore, getTriggerSystem, getScheduler, getMonitorManager, updateWidget, maybeBootstrapTaskLoop, isTaskSystemReady, onDynamicLoopActivated, cancelOrchestration, } = options;
    pi.registerTool({
        name: "LoopCreate",
        label: "LoopCreate",
        ...
        async execute(_toolCallId, params) {
            ...
            let backlogEventSource;
            if (trigger.type === "event")
                backlogEventSource = trigger.source;
            else if (trigger.type === "hybrid")
                backlogEventSource = trigger.event.source;
            let backlogError;
            if (taskBacklog && autoTask)
                backlogError = "taskBacklog loops cannot enable autoTask; backlog workers adopt existing tasks instead of creating more.";
            else if (taskBacklog && recurring === false)
                backlogError = "taskBacklog loops must be recurring.";
    ...
    pi.registerTool({
        ...
        execute() {
            ...
            for (const entry of loops) {
                ...
                line += ` (${triggerDesc})`;
                if (activity)
                    line += ` [activity:${activity.status} ${formatCompactWorkflowDuration(activity.activityMs)}]`;
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params) {
            ...
            if (existing?.orchestration) {
                const orchestrationAction = action === "pause" ? "pause" : "delete";
