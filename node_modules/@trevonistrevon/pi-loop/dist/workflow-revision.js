...
export function validatePersistedWorkflowRevision(revision, expectedRevision) {
    if (!Check(WorkflowDefinitionRevisionSchema, revision))
        return "history record is invalid";
    if (revision.revision !== expectedRevision)
        return "history is not contiguous";
...
function pathExists(definition, start, predicate) {
    const queue = [start];
    ...
    while (queue.length > 0) {
        ...
        if (!state)
            continue;
        if (predicate(stateId, state))
            return true;
        queue.push(...Object.values(state.on ?? {}));
    }
    return false;
}
function validatePreconditions(run, input) {
    const currentRevision = run.definitionRevision ?? 1;
    const revisionHistory = run.revisionHistory ?? [];
    if (input.expectedRevision !== currentRevision) {
        return rejected("revision_conflict", `Workflow revision conflict: expected revision ${input.expectedRevision}, current revision ${currentRevision}.`, { expectedRevision: input.expectedRevision, currentRevision });
    }
    if (input.expectedState !== run.currentState || input.expectedTransitionSeq !== run.transitionSeq) {
        return rejected("run_conflict", `Workflow run changed: expected ${input.expectedState}@${input.expectedTransitionSeq}, current ${run.currentState}@${run.transitionSeq}.`, {
...
function authorize(run, actor, at) {
    ...
    if (!active.lease)
        return rejected("execution_unowned", "Workflow execution is unowned; call WorkflowClaim before revising.");
...
function validateChangeIdentity(change, stateChanges, edgeChanges) {
...
function addState(definition, change, summary) {
...
function validateRevisedLimits(run, change) {
...
function validateReissueLimits(run, change) {
    if (change.maxAttempts !== undefined && change.maxAttempts < (run.attemptsByState[change.stateId] ?? 0)) {
...
function validateStateRevision(run, state, change) {
    if (!state)
        return rejected("state_conflict", `Workflow state "${change.stateId}" does not exist.`, { stateId: change.stateId });
    ...
    if (fields.length === 0)
        return rejected("invalid_patch", `revise_state "${change.stateId}" has no changes.`, { stateId: change.stateId });
...
function reviseState(run, definition, change, summary) {
    ...
    if (change.prompt !== undefined)
        revised.prompt = change.prompt;
    if (change.task !== undefined)
        revised.task = structuredClone(change.task);
    if (change.loop !== undefined)
        revised.loop = structuredClone(change.loop);
    if (change.maxAttempts !== undefined)
        revised.maxAttempts = change.maxAttempts;
...
function reissueState(run, definition, change, summary) {
    if (change.stateId !== run.currentState) {
    ...
    if (limitError)
        return limitError;
    const revised = { ...state, prompt: change.prompt };
    if (change.task !== undefined)
        revised.task = structuredClone(change.task);
    if (change.loop !== undefined)
        revised.loop = structuredClone(change.loop);
...
function applyStateChanges(run, definition, changes, summary) {
    const additions = changes.filter((change) => change.op === "add_state");
    for (const change of additions) {
        const error = addState(definition, change, summary);
        if (error)
            return error;
    }
    const revisions = changes.filter((change) => change.op === "revise_state");
    for (const change of revisions) {
        const error = reviseState(run, definition, change, summary);
        if (error)
            return error;
    }
    const reissues = changes.filter((change) => change.op === "reissue_state");
...
function edgeContext(definition, from, to) {
    const source = definition.states[from];
    if (!source)
        return rejected("edge_conflict", `Workflow transition source "${from}" does not exist.`, { stateId: from });
    if (source.terminal)
        return rejected("edge_conflict", `Terminal state "${from}" cannot declare transitions.`, { stateId: from });
...
function addTransition(definition, change, summary) {
    const context = edgeContext(definition, change.from, change.to);
    if ("applied" in context)
        return context;
...
function redirectTransition(definition, change, summary) {
    const context = edgeContext(definition, change.from, change.to);
    if ("applied" in context)
        return context;
    const { source, transitions } = context;
    ...
    if (currentTarget !== change.expectedTo) {
        return rejected("edge_conflict", `Transition "${change.from}.${change.outcome}" expected target "${change.expectedTo}" but found "${currentTarget ?? "missing"}".`);
    }
    if (change.to === change.expectedTo)
        return rejected("invalid_patch", `Redirect "${change.from}.${change.outcome}" is a no-op.`);
    transitions[change.outcome] = change.to;
    definition.states[change.from] = { ...source, on: transitions };
...
function applyEdgeChanges(definition, changes, summary) {
    for (const change of changes) {
        let error;
        if (change.op === "add_transition")
            error = addTransition(definition, change, summary);
        if (change.op === "redirect_transition")
            error = redirectTransition(definition, change, summary);
        if (error)
            return error;
    }
    return undefined;
}
function validateGraph(run, definition, summary) {
    ...
    for (const stateId of summary.addedStates) {
        ...
        const rejoins = pathExists(definition, stateId, (candidate, state) => state.terminal !== undefined || (candidate !== stateId && priorStates.has(candidate)));
        if (!rejoins)
            return rejected("graph_invalid", `New state "${stateId}" does not rejoin prior workflow work or a terminal state.`, { stateId });
    }
    for (const edge of summary.redirectedTransitions) {
        if (!pathExists(definition, edge.to, (candidate) => candidate === edge.fromTarget)) {
            return rejected("dependency_not_preserved", `Redirect "${edge.from}.${edge.outcome}" must retain a path to prior target "${edge.fromTarget}".`);
        }
    }
    return undefined;
}
function replaceCurrentExecution(run, definition, nextRevision, reason, at) {
...
export function reviseWorkflowRun(run, input, at) {
    const preconditionError = validatePreconditions(run, input);
    if (preconditionError)
        return preconditionError;
    const actor = input.actor;
    if (!actor)
        return rejected("actor_required", "Workflow revision requires an active session runtime.");
    const authorizationError = authorize(run, actor, at);
    if (authorizationError)
        return authorizationError;
    const reason = input.reason.trim();
    if (!reason || reason.length > 1000)
        return rejected("invalid_patch", "Workflow revision reason must contain 1-1000 characters.");
