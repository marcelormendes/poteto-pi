...
import { displayRows } from "../tools/tool-result.js";
import { getWorkflowOutcomeAvailability } from "../workflow-reducer.js";
function unavailableOutcomeLabel(item) {
    if ("reason" in item)
        return `${item.outcome} (${item.targetState} unbounded self-loop)`;
    return `${item.outcome} (${item.targetState} exhausted ${item.maxAttempts})`;
}
function boundedAge(end, start) {
    return Math.max(0, end - start);
}
export function formatCompactWorkflowDuration(ms) {
    const bounded = Math.max(0, ms);
    if (bounded < 60_000)
        return `${Math.round(bounded / 1_000)}s`;
    if (bounded < 3_600_000)
        return `${Math.round(bounded / 60_000)}m`;
    return `${Math.round(bounded / 3_600_000)}h`;
}
export function deriveWorkflowActivity(entry, now = Date.now()) {
...
export function workflowTimingSummaryLine(entry, now = Date.now()) {
    const activity = deriveWorkflowActivity(entry, now);
...
export function workflowAttemptLabel(entry) {
    const workflow = entry.workflow;
    if (!workflow)
        return "?";
    const state = workflow.definition.states[workflow.currentState];
...
export function workflowLeaseLabel(entry, now = Date.now()) {
    const workflow = entry.workflow;
    if (!workflow)
        return "unavailable";
    const execution = workflow.activeExecution;
    if (!execution)
        return workflow.definition.states[workflow.currentState]?.task ? "missing" : "not required";
    if (!execution.lease)
        return "unowned";
...
export function workflowDisplayDetails({ entry, action, tone, summary, extra = [], }) {
    const workflow = entry.workflow;
    if (!workflow) {
...
export function formatWorkflowInspection(entry) {
    const workflow = entry.workflow;
    if (!workflow)
        return `#${entry.id}: ${entry.prompt}`;
    const state = workflow.definition.states[workflow.currentState];
    const availability = getWorkflowOutcomeAvailability(workflow);
    const now = Date.now();
    const lines = [
