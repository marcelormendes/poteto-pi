...
import { createSubagentOrchestrationRuntime } from "./runtime/subagent-orchestration-runtime.js";
import { createTaskBacklogRuntime } from "./runtime/task-backlog-runtime.js";
import { createTaskProviderRuntime } from "./runtime/task-provider-runtime.js";
...
export default function (pi) {
    ...
    function createScheduler(loopStore) {
        return new CronScheduler(loopStore, (entry, origin) => onLoopFire(entry, undefined, origin), (entry, disposition) => emitLoopExpired(entry, disposition, "scheduler", "expires_at"), isCurrentExtensionContext);
    }
    ...
    let taskProvider;
    const activeTaskBacklogWakes = new Set();
    ...
    const notificationRuntime = createNotificationRuntime({
        ...
        onLoopNotificationDelivered: ({ loopId, orchestrationWakeSequence }) => {
            if (orchestrationWakeSequence !== undefined)
                orchestrationRuntime?.acknowledgeWake(loopId, orchestrationWakeSequence);
        },
        debug,
    });
    ...
    async function maybeBootstrapTaskLoop(entry) {
        if (!entry.recurring || !entry.taskBacklog)
            return false;
    ...
    // ── Loop fire handler ──
    function isCurrentExtensionContext() {
        try {
    ...
    function emitLoopExpired(entry, disposition, source, reason, generation = sessionGeneration) {
        if (generation !== sessionGeneration || !isCurrentExtensionContext())
            return;
    ...
    function emitLoopFire(entry, monitor, orchestrationWakeSequence) {
    ...
    function onLoopFire(entry, monitor, origin = monitor ? "monitor" : "dynamic", promptOverride) {
        if (!isCurrentExtensionContext())
            return;
        ...
        if (atMaxFires(current)) {
            ...
            if (current.workflow || current.taskBacklog)
                store.pause(current.id, "controller_limit", "loop fire cap reached");
            else
        ...
        if (isTaskBacklog)
            activeTaskBacklogWakes.add(current.id);
        ...
        if (atMaxFires(firedEntry)) {
            ...
            if (firedEntry.workflow || firedEntry.taskBacklog)
                store.pause(firedEntry.id, "controller_limit", "loop fire cap reached");
            else
        ...
        if (current.autoTask) {
            taskProvider?.autoCreateTask(current).then((taskId) => {
                if (taskId)
                    debug(`loop #${current.id} → task #${taskId}`);
            });
        }
    ...
    pi.events.on("monitor:started", async (event) => {
        const sourceGeneration = sessionGeneration;
        const data = event;
        if (!data.monitorId || !data.command)
            return;
    ...
    pi.events.on("monitor:finished", (event) => {
        const data = event;
        if (data.monitorId)
            discardMonitorStarted(data.monitorId);
    });
    registerLoopTools({
        pi,
        getStore: () => store,
    ...
    function handleWorkflowMonitorWait(entry) {
