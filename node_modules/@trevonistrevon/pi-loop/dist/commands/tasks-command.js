import { createTask, deleteTask, taskMutationRejectionMessage, updateTask, } from "../runtime/task-mutations.js";
export function registerTasksCommand(options) {
    const { pi, getNativeTaskStore, evaluateTaskBacklog, updateWidget } = options;
    function mutationContext(taskStore) {
        return { pi, taskStore, evaluateTaskBacklog, updateWidget };
    }
    async function createNativeTask(subject, description) {
    ...
    async function viewNativeTasks(ui) {
        ...
        if (action === "x Delete") {
            ...
            if (!result.applied)
                rejection = taskMutationRejectionMessage(task.id, result);
        }
        else {
