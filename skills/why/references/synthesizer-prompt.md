...
Build the synthesizer pass's prompt from this template; fill in the placeholders.
...
1. **Read all pass findings.** They gathered raw evidence, not conclusions. You weigh it.
2. **Reconcile overlapping findings.** Multiple passes may have cited the same PR, ticket, or doc. Merge into a single, authoritative reference.
...
One line per pass: `Pass N — <source> — <model> — PASS/ISSUES/BLOCKED`, matching the status lines from the investigator findings.
