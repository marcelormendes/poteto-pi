...
List the available MCP servers first: project `.pi/mcp.json`, user `~/.pi/agent/mcp.json` (global), or `/mcp` in-session for status; MCP tools appear in the session tool list as `mcp__<server>__<tool>`. If no config file exists, the available-tools list is the source of truth for what is wired up.
