...
const LANES = "Ten lanes on `fast mechanical model (setup default)` at the PR head";
const SUB_BLOCKS = [
	"Depends on.",
	"Files.",
	"Build.",
...
const PROGRAM_H3 = ["Arm the program", "Run owner passes", "PR mechanics", "Verdict and merge", "Boot recipe"];
const PROGRAM_MARKERS = ["run objective", "git show origin/main:", /30[- ]minute/, "status message"];
...
if (!program) fail(1, 'no "## Program checklist" section');
	...
	for (const name of PROGRAM_H3) {
		const at = h3s.findIndex((t, i) => i >= pos && t.startsWith(name));
