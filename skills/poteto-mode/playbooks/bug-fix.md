...
3. Plan the fix. If it crosses a function boundary, `architect` first. Run the implementation pass using your configured bug-fix model (default `strongest judgment model (setup default)`) with a specific scope; review the diff before accepting.
...
Investigation runs the `how` + `why` passes sequentially.
