...
3. Migrate one component at a time. Each is an independent artifact, so run one pass per component, each in its own git worktree (the **separate-before-serializing-shared-state** principle skill). Shared primitives migrate first as a blocking phase.
