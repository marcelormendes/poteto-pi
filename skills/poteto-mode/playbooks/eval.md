...
4. **Run N sequential candidate passes** on different models per the **arena** skill's Phase B. Each works in its own sanitized dir; same prompt to each. Switch the session model between passes and record which model each pass ran under.
5. **Run one blinded judge pass** on a different model family per the **arena** skill's Phase C. Judge sees outputs by sanitized label and the rubric, never a model name.
