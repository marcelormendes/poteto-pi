...
description: "Apply to any non-trivial work, not just bulk work: edits, migrations, analyses, checks. Build the tool that does it or proves it (codemod, script, generator, or a skill your passes follow) instead of working by hand. The tool is the artifact a reviewer can rerun."
...
- A deterministic lever beats hand application. If the tool can process every unit in one pass, run it yourself; don't work through passes that hand-apply what a script can do.
- When work spans several sequential passes, write the lever as a skill every pass reads: the recipe, the verification contract, and the do-not-touch fences in one artifact, so every pass inherits the same hardened version instead of re-explaining it per brief and watching each one drift. Keep it outside the pass's write scope so it can't quietly edit the contract.
- Applying this principle produces a file. If you cited it and there is no codemod, script, generator, or pass skill in the diff, you didn't apply it.
