...
Build each explorer pass's prompt from this template. Fill in the placeholders.
...
Start by finding the relevant code. Use `find` and `ls` to locate directories and files, `grep` to find key symbols, `read` to understand the actual implementation. Don't guess from names. Read the code.
...
Start with a status line: **PASS** (findings complete), **ISSUES** (partial findings; name what you couldn't cover), or **BLOCKED** (couldn't reach the code at all; say why). The lead skips and notes a BLOCKED pass rather than treating it as coverage — don't pad the output to look complete.
