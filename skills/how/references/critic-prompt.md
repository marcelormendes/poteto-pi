...
Build each critic pass's prompt from this template. Fill in the placeholders.
...
Start with a status line: **PASS** (full critique delivered), **ISSUES** (critique delivered, but some areas unverified; name which), or **BLOCKED** (could not read the relevant code; say why). The lead skips and notes a BLOCKED pass rather than treating it as a verdict — don't pad the output to look complete.
