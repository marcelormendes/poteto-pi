...
You are writing an architectural explanation for a senior engineer. Multiple explorer passes have traced different slices of the codebase and gathered findings. Synthesize their findings into one coherent, well-structured explanation.
...
You have read access to the codebase to check anything, clarify a detail, or fill a gap. Use `read`, `grep`, and `find` as needed; edit nothing. The explorer passes did the heavy lifting, so you shouldn't need to re-explore from scratch.
...
- If the explorer passes flagged open questions or gaps, acknowledge them honestly rather than papering over them
